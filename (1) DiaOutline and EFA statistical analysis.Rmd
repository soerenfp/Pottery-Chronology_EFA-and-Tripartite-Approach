...
#(1) Produce a character vector of the names of files or directories in the named directory with the list.files() arguement. The pattern = "\\.txt$" command is not to be changed, and I think it tells that the raw data is in a .txt format
...
#(2) If filenames are consistently named with the same character serating factors, and with every individual including its belonging levels, this function returns a data.frame from it that can be passed to Out, Opn, Ldk objects.
...
#(4) Import the coordinates from the .txt files
...
#NOTE: New problem below -> Error in coo_check.default(X[[i]], ...) : do not know how to turn into a coo
...
#NOTE: stack(allDiatomOutlines) now there is one being really large and the rest is pretty small. I need to change that before uploading it to DiaOutline
...
#NOTE: "Conflicting situations such as those suggested by this familiar saying are actually a recurrent issue in morphometrics: what is the right number of harmonics? Unfortunately, no objecttive criterion exists so far, and the criterion used usually depends on the scope of the study." (Bonhomme et al. 2014)
