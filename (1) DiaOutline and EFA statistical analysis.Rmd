...
Caple, J., J. Byrd, C. N. Stephan. 2017. Elliptical Fourier analysis:             fundamentals, applications, and value for forensic anthropology. Int J Legal    Med (2017) 131: 1675–1690. https://doi:10.1007/s00414-017-1555-0 
...
####((2)) Create a Data Set from .txt DiaOutline file (not needed)
```{r, echo=TRUE}
m1 <- as.matrix(read.table("C:/Program Files/DiaOutline_working directory/DiaOutline_trial trace 01.txt", sep = "", header = FALSE))
m2 <- as.matrix(read.table("C:/Program Files/DiaOutline_working directory/DiaOutline_trial trace 02.txt", sep = "", header = FALSE))
m3 <- as.matrix(read.table("C:/Program Files/DiaOutline_working directory/DiaOutline_trial trace 03.txt", sep = "", header = FALSE))

DiaOutlineDataset <- c(m1, m2, m3)
```
NOTE 1: The .txt file from DiaOutline contains x and y coordinates separated by space. I think we have to break it up into a matrix with two columns (?) with the as.matrix() function. 
(sep = "")        -> separates space columns
(header = FALSE)  -> describes that the first row does NOT contain header                            information

NOTE 2: Perhaps I dont need to separate the .txt file into columns.. Maybe the next code chunk will handle the data set?

####(2.2.1) OUTLINE ANALYSIS - Species data
```{r, echo=TRUE}
#Following chunk is by me (old, see below)

lf <- list.files("E:/Program Files/DiaOutline_working directory/DiaOutline_trial trace 01.txt", pattern = NULL, full.names=TRUE)

#Problem comes below: "Error in table(lfl) : nothing to tabularise" 

lf1<-lf_structure(lf,  split = "E:/Program Files/DiaOutline_working directory/DiaOutline_trial trace 01.txt", trim.extension = FALSE)
lf2<-data.frame(substr(lf1$V2, 1, 5))
names(lf2)[1] <- "Type"
```

```{r, echo=TRUE}
#Following chunk by me (new, came further)

lf <- list.files("C:/Program Files/DiaOutline_working directory/DiaOutlineDataset/", pattern = "\\.txt$", full.names=TRUE)  
lf1<-lf_structure(lf,  split = "C:/Program Files/DiaOutline_working directory/DiaOutlineDataset/", trim.extension = FALSE)
lf2<-data.frame(substr(lf1$V2, 1, 5))
names(lf2)[1] <- "Type"
...
#In the following comes the problem
