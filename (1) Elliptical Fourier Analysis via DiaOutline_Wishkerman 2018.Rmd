...
#NOTE: Perhaps my problem about "do not know how to turn into a coo" in part 6 below already starts here? Perhaps the path is wrong/to long or some of the .txt files are crooked.
...
#NOTE: New problem below -> "Error in coo_check.default(X[[i]], ...) : do not know how to turn into a coo" -> The problem is that some of the .txt files might be crooked. When I try to run the codes with just my original three trial traces, it works. When I try with the new 20 .txt files this error occurs.
#=> Problem solved! My trial trace 05.txt file was crooked and removing it solved the problem!
...
#NOTE: stack(allDiatomOutlines) now there is one vessel (trial trace 04) being really large and the rest is pretty small. I need to change that before uploading it to DiaOutline
...
allDiatomOutlines.f <- efourier(allDiatomOutlines, norm = TRUE, nb.h=32) #Num. of harmonics set to 32
```
...
plot(allDiatomOutlines.p, chull.filled = TRUE, stars = TRUE, title="All data PCA") #Supplement 10 in the paper.
...
#dev.off()
```
Hmm, I do not know if this looks right. It looks like only one vessel is in the plot (trial trace 04). After I removed trial trace 04 it looks better, but I fear it is still just one vessel (trial trace 06 or 07) which is in the plot.
